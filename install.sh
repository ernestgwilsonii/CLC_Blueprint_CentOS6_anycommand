...
#    Runs any arbitrary command on CentOS 6x and emails the output 
# Version 1.0 November 16, 2015 ErnestGWilsonII@gmail.com
#         anycommand_centos6x.zip
################################################################################
...
# Sends any command in $COMMAND
/bin/echo "################################################################################" >> $REPORTFILE
/bin/echo "# Command Output:" >> $REPORTFILE
/bin/echo "#################" >> $REPORTFILE
/bin/echo " " >> $REPORTFILE
/bin/echo " Command sent was: $COMMAND" >> $REPORTFILE
/bin/echo " " >> $REPORTFILE
/bin/echo "  Command results:" >> $REPORTFILE
/bin/echo " " >> $REPORTFILE
$COMMAND >> $REPORTFILE 2>&1
/bin/echo "" >> $REPORTFILE
/bin/echo "################################################################################" >> $REPORTFILE
/bin/echo "" >> $REPORTFILE
/bin/echo "" >> $REPORTFILE
...
/bin/cat $LOGFILE | /bin/mailx -s "Blueprint Report: Any Command Results for CentOS 6x on: $THISHOST at $1" -a $REPORTFILE $EMAILREPORTTO